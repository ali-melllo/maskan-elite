...
import { CheckIcon, Key, Crown } from "lucide-react";
            ...
            <div className="flex flex-col items-center justify-center max-w-2xl mx-auto">
                <Container>
                    <div className="flex flex-col items-center text-center max-w-2xl mx-auto">
                        <h2 className="text-2xl md:text-4xl lg:text-5xl font-heading font-medium !leading-snug mt-6">
                            Membership Tiers <br className="hidden lg:block" /> <span className="font-subheading italic">& Access</span>
                        </h2>
                        <p className="text-base md:text-lg text-center text-accent-foreground/80 mt-6">
                            By invitation only. Each tier is personally vetted by Arshiya Maskan to ensure absolute integrity within the ecosystem.
                        </p>
                    </div>
                </Container>
                ...
                <Container delay={0.2}>
                    <div className="flex items-center justify-center space-x-4 mt-6">
                        <span className="text-base font-medium">Athlete</span>
                        ...
                        <span className="text-base font-medium">Institution</span>
                    </div>
                </Container>
            </div>
    ...
    return (
        <div className={cn(
            ...
            <div className="p-4 md:p-8 flex rounded-t-2xl lg:rounded-t-3xl flex-col items-start w-full relative">
                <h2 className="font-medium text-xl text-foreground pt-5 flex items-center gap-2">
                    {plan.title === "Core" && <Key className="size-5 text-primary" />}
                    {plan.title === "Mastermind" && <Crown className="size-5 text-primary" />}
                    {plan.title}
                </h2>
                <h3 className="mt-3 text-3xl font-medium md:text-5xl">
                    <NumberFlow
                        value={billPlan === "monthly" ? plan.monthlyPrice : plan.annuallyPrice}
                        suffix={billPlan === "monthly" ? "" : ""}
                        format={{
                            currency: "USD",
                            style: "currency",
            ...
            <div className="flex flex-col items-start w-full px-4 py-2 md:px-8">
                <Button size="lg" variant={plan.title === "Mastermind" ? "blue" : "white"} className="w-full">
                    {plan.buttonText === "Start Free Trial" ? "Request Invitation" : 
                     plan.buttonText === "Get Started" ? "Apply for Access" : plan.buttonText}
                </Button>
                <div className="h-8 overflow-hidden w-full mx-auto">
                    <AnimatePresence mode="wait">
                        ...
                        >
                            {billPlan === "monthly" ? (
                                "Athlete Member Track"
                            ) : (
                                "Institutional Partner Track"
                            )}
                        </motion.span>
                    </AnimatePresence>
                </div>
